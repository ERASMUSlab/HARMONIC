...
#'   last column of HARMONIC_SEPERATE output.
...
#' @param dynamic_analyisis "F" for single/multi heatmap using HARMONIC_SEPERATE,
#'   "T" for dynamic analysis heatmap using HARMONIC_SEPERATE_forMULTI_range.
#'
...
#' # )
#' }
HARMONIC_HeatMap <- function(filepath,
                          DEG_list_name,
                          trends,
                          condition = "ALL",
                          width,
    ...
    if (TXTbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.txt"))[1, 1],
                             "HARMONIC_SEPERATE.txt")
    }
    if (CSVbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.csv"))[1, 1],
                             "HARMONIC_SEPERATE.csv")
    }
    if (BEDbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.bed"))[1, 1],
                             "HARMONIC_SEPERATE.bed")
    }
    HARMONIC_SEPERATE_path <- OUTPUTpath

    if (TXTbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.txt"))[1, 1],
                             "HARMONIC_SEPERATE_forMULTI_range.txt")
    }
    if (CSVbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.csv"))[1, 1],
                             "HARMONIC_SEPERATE_forMULTI_range.csv")
    }
    if (BEDbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.bed"))[1, 1],
                             "HARMONIC_SEPERATE_forMULTI_range.bed")
    }
    ...
    if (dynamic_analyisis == "F") {
        ...
        input_SEPERATE <- HARMONIC_SEPERATE_path
    ...
    if (dynamic_analyisis == "T") {
        ...
        input_SEPERATE <- HARMONIC_SEPERATE_forMULTI_range_path
