#' Call DEGs with DESeq2 and write per-contrast DEG files for HARMONIC
#'
...
HARMONIC_DEGcalling = function(filepath,
                            type = "standard",
                            full_condition,
                            number_of_rep,
                            mango_design){

    HARMONIC_CMD = paste0(": > ", filepath, "/input_DEG_list.txt")
    base::system(HARMONIC_CMD, intern = TRUE)
    ...
    for(e in 1:nrow(mango_design_DF)){
        ...
        HARMONIC_CMD = paste0("echo ", mainPATH, " >>", filepath, "/input_DEG_list.txt")
        base::system(HARMONIC_CMD, intern = TRUE)
