...
#' @param HARMONIC_PREPROCESSING_list_path Output path to write the list of generated
#'   HARMONIC preprocessing result files.
...
#' @param filepath Base directory where \code{CODE/init_HARMONIC_PREPROCESSING.sh} exists.
#'
...
HARMONIC_PREPROCESSING = function(DEG_list_path,
                               GO_list_path,
    ...
    HARMONIC_PREPROCESSING_CMD = paste0("bash ",
                                     filepath,
                                     "/CODE/init_HARMONIC_PREPROCESSING.sh",
                                     " ",GO_list_path,
                                     " ",process_num,
                                     " ",core)

    system(HARMONIC_PREPROCESSING_CMD, intern = T)
    
    HARMONIC_PREPROCESSING_list = GO_list
    ...
    for(i in 1:nrow(GO_list)){
        ...
        if(TXTbased>0){OUTPUTpath = paste0(as.data.frame(strsplit(sublist[,1], split = ".txt"))[1,1],"_HARMONIC_PREPROCESSING.txt")}
        if(CSVbased>0){OUTPUTpath = paste0(as.data.frame(strsplit(sublist[,1], split = ".csv"))[1,1],"_HARMONIC_PREPROCESSING.csv")}
        if(BEDbased>0){OUTPUTpath = paste0(as.data.frame(strsplit(sublist[,1], split = ".bed"))[1,1],"_HARMONIC_PREPROCESSING.bed")}

        HARMONIC_PREPROCESSING_list[i,1] = OUTPUTpath
        }

    utils::write.table(HARMONIC_PREPROCESSING_list,
                file = HARMONIC_PREPROCESSING_list_path,
                col.names=T, row.names=F, quote=F,sep="\t")
    }
