...
#'
#' Visualize tree-level summaries from `HARMONIC_SEPERATE` output for a single-case
...
#' # )
#' }
HARMONIC_TREE_PLOT_forSINGLE <- function(filepath,
                                      trends,
                                      width,
                                      height,
                                      plot,
    ...
    if (TXTbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(DEG_list_pathDF[, 1], split = "input_DEG_list.txt"))[1, 1], "HARMONIC_PREPROCESSING_list.txt")
    }
    if (CSVbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(DEG_list_pathDF[, 1], split = "input_DEG_list.csv"))[1, 1], "HARMONIC_PREPROCESSING_list.csv")
    }
    if (BEDbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(DEG_list_pathDF[, 1], split = "input_DEG_list.bed"))[1, 1], "HARMONIC_PREPROCESSING_list.bed")
    }
    ...
    if (TXTbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.txt"))[1, 1], "HARMONIC_SEPERATE.txt")
    }
    if (CSVbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.csv"))[1, 1], "HARMONIC_SEPERATE.csv")
    }
    if (BEDbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.bed"))[1, 1], "HARMONIC_SEPERATE.bed")
    }
    HARMONIC_SEPERATE_path <- OUTPUTpath

    if (TXTbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.txt"))[1, 1], "HARMONIC_SEPERATE_forMULTI_range.txt")
    }
    if (CSVbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.csv"))[1, 1], "HARMONIC_SEPERATE_forMULTI_range.csv")
    }
    if (BEDbased > 0) {
        OUTPUTpath <- paste0(as.data.frame(strsplit(HARMONIC_PREPROCESSING_list_pathDF[, 1], split = "HARMONIC_PREPROCESSING_list.bed"))[1, 1], "HARMONIC_SEPERATE_forMULTI_range.bed")
    }
    ...
    HARMONIC_SEPERATE_INPUT <- HARMONIC_SEPERATE_path
    SEPERATE_INPUT <- utils::read.table(HARMONIC_SEPERATE_INPUT, sep = "\t", header = TRUE, fill = TRUE)
